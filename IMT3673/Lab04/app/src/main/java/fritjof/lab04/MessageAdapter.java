...
public class MessageAdapter extends ArrayAdapter<Message> {
    ...
    public MessageAdapter(Context context, int id, boolean withName) {
        super(context, id);
    ...
    public View getView(int pos, View convertView, @NonNull ViewGroup container) {
        ...
        if (messageRow.user.equals(main.nickname)) {
        ...
        if (withName) {
            user.setText(messageRow.user);
        } else {

            // Gone so that it doesn't take up space
